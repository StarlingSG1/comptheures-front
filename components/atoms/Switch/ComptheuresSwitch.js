...
export function ComptheuresSwitch({ children, className="",getRecapTimes, comptheuresSwitchState, setComptheuresSwitchState }) {
    ...
    return (
        <div className={joinClasses(className,"h-[54px] w-full relative dark:bg-blue-dark bg-blue rounded-xl mt-10")}>
            <button onClick={() => {setComptheuresSwitchState(false)}} className={`${comptheuresSwitchState ? "text-gray" : "font-bold text-blue"} h-full w-1/2 relative z-10`}>Compter ses heures</button>
            <button onClick={() => {setComptheuresSwitchState(true);}} className={`${comptheuresSwitchState ? "font-bold text-blue" : "text-gray"} h-full w-1/2 relative z-10`}>Récapitulatif</button>
